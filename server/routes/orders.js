...
router.patch('/:id/status', auth, requireRole('Admin'), async (req, res) => {
  ...
  try {
    ...
    await conn.query(
      'UPDATE PurchaseOrder SET Status = ? WHERE OrderID = ?',
      [status, req.params.id]
    );

    if (status === 'Delivered') {
      await conn.query(
        'UPDATE PurchaseOrder SET ActualDeliveryDate = CURDATE() WHERE OrderID = ?',
        [req.params.id]
      );
...
router.delete('/:id', auth, requireRole('Admin'), async (req, res) => {
  try {
    await pool.query(
      "UPDATE PurchaseOrder SET Status = 'Cancelled' WHERE OrderID = ?",
      [req.params.id]
    );
